...
ISR(PCINT2_vect) { // handle pin change interrupt for D0 to D7 here
  ...
  if (!(PIND & (1 << PD2)) &&(changedbits&(1<< PIND2))&&(PCMSK2 & (1 << PCINT18))) {
    ...
    if (sensor_sound.State == state::zero) {//First to arrive is D2
      ...
      TCCR2B |= 0b00000001;
      sensor_sound.delay_time_sample[sensor_sound.counter_sample]= ((sensor_sound.timer_counter << 8) + TCNT2) / 16;
      PCMSK2 &= ~(1 << PCINT18);
...
void  determine_time_delay() {
  ...
  for(int i=0;i<samples_number;i++){
    if((sensor_sound.delay_time_sample[i]>0)&&(sensor_sound.delay_time_sample[i]<=time_delay_maximum)){
